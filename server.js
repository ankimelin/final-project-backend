...
app.post('/exhibitions', async (req, res) => {
  try {
    const { title, artists, museum, startDate, endDate } = req.body
    const exhibition = await new Exhibition({ title, artists, museum, startDate, endDate }).save()
...
app.patch('/exhibitions/:id', async (req, res) => {
  try {
    const { id } = req.params
    const { title, museum, artists, startDate, endDate } = req.body
    const exhibition = await Exhibition.updateOne({ _id: id }, { title, museum, artists, startDate, endDate }, { runValidators: true })
